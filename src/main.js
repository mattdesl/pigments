...
import { K, S } from "../data/adjusted-primaries-optimized-feb13.json";
...
const DEFAULT_INTERPOLATION_PARAMS = [
  [0, 0],
  [0, 0],
  [0, 0],
  [0, 0],
...
const toData = (weights, params, silu = false) => ({
  params,
  silu,
...
//
const NUM_PIGMENTS = 5;
const dataReLU = toData(weights, [3, 16, 16, NUM_PIGMENTS], false);
...
function adjustableSmoothStepF(x, p, c) {
  return (x * (x + c * p)) / (x + c * p);
}

function adjustableSmoothStep(x, p = 0.5, g = 1.5) {
  const c = (g - 1) / (2 - g);
  return x < 0 || x > 1
    ? 0
    : x < p
    ? adjustableSmoothStepF(x, p, c)
    : 1 - adjustableSmoothStepF(1 - x, 1 - p, c);
}

function smoothstep(x, lowBound = 0, highBound = 1) {
  x = clamp((x - lowBound) / (highBound - lowBound), 0.0, 1.0);
  return x * x * (3.0 - 2.0 * x);
}

// 0..255 to index 3D
function rgb_to_index_3D(srgb, N) {
  const stepSize = 255.0 / (N - 1.0);
  return srgb.map((v) => Math.floor(v / stepSize));
}

function rgb_to_index_1D(srgb, N) {
  return index_3D_to_index_1D(rgb_to_index_3D(srgb, N), N);
}

function index_3D_to_index_1D(index, N) {
  // flatIndex=(iR×N×N)+(iG×N)+iB
  return index[0] * N * N + index[1] * N + index[2];
}

function index_3D_to_rgb(index, N) {
  const stepSize = 255.0 / (N - 1.0);
  return index.map((v) => clamp(Math.floor(v * stepSize), 0, 255));
}

function index_1D_to_3D(index, N) {
  let iB = index % N;
  let tmp = (index - iB) / N;
  let iG = tmp % N;
  let iR = (tmp - iG) / N;
...
function sample(data, x, y, z, N, channels = 4) {
  x = clamp(x, 0, N - 1);
  y = clamp(y, 0, N - 1);
  z = clamp(z, 0, N - 1);
...
function getCMYKFromLUT(srgb, N, dataRGBA) {
  const step = N - 1;
...
function rgb_to_float_indices(srgb, N) {
  const stepSize = 255.0 / (N - 1.0);
  return srgb.map((v) => v / stepSize);
}

// Get indices and fractional parts for interpolation
function rgb_to_indices_and_fractions(srgb, N) {
...
function sample_lut(xyz, N, data, channels = 4) {
...
function linearToConcentration(oklab0, oklab1, t, exponent = 2.0) {
...
function evalPolynomial(pigments) {
...
function lerpLatents2(oklab0, oklab1, latent0, latent1, t, linear = false) {
  const rcount = 3;
  const gamma = 2.2;
...
function OKLtoConcentration(oklab0, oklab1, t) {
...
function getCMYKFromLUT_Trilinear(srgb, N, dataRGBA, channels = 4) {
  const step = N - 1;
...
function neuralRGBToLatent(rgb) {
...
function neuralRGBToLatentPolynomial(rgb) {
  const oklab = Color.convert(
    rgb.map((n) => n / 0xff),
    Color.sRGB,
    Color.OKLab
...
function neuralLatentToRGB(latent) {
  const rcount = 3;
...
function neuralLatentToRGBPolynomial(latent) {
  const rcount = 3;
  const pcount = latent.length - rcount;
  const weights = latent.slice(0, pcount);
  const residual = latent.slice(pcount);
...
function oklab_to_srgb_mapped(oklab, residual = [0, 0, 0]) {
...
function LUTMixer(K, S, N, dataRGBA) {
  // const tmp3A = [0, 0, 0];
  // const tmp3B = [0, 0, 0];
